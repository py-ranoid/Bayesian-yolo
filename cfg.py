...
def save_cfg(blocks,destination):
    ...
    content =  '\n\n'.join([block2text(x) for x in blocks])
...
def load_conv(buf, start, conv_model):
    num_w = conv_model.weight.data.numel()
    num_b = conv_model.bias.data.numel()
...
def load_conv_bayes(buf, start, conv_model):
    ...
    start = start + num_b
    conv_model.weight_mu.data.copy_(torch.from_numpy(buf[start:start + num_w]).view(conv_model.weight_mu.data.shape))
    start = start + num_w
    return start
...
def load_conv_bn_bayes(buf, start, conv_model, bn_model):
    num_w = conv_model.weight_mu.numel()
    num_b = bn_model.bias.numel()
    bn_model.bias.data.copy_(torch.from_numpy(buf[start:start + num_b]))
    start = start + num_b
    ...
    bn_model.running_var.copy_(torch.from_numpy(buf[start:start + num_b]))
    start = start + num_b
    conv_model.weight_mu.data.copy_(torch.reshape(torch.from_numpy(buf[start:start+num_w]),
                                                  (conv_model.weight_mu.shape[0],conv_model.weight_mu.shape[1], conv_model.weight_mu.shape[2],conv_model.weight_mu.shape[3])
                                                  )
                                    );
    start = start + num_w
    return start
